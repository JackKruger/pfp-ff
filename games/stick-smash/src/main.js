...
import { createPfpRuntime } from "./pfp/PfpRuntime";
import { PfpControls } from "./input/PfpControls";
import { installPfpExternalMatch } from "./pfp/externalMatch";
import { buildStickSmashResult } from "./pfp/results";
...
async function boot() {
  ...
  try {
    ...
    document.getElementById("loading").textContent =
      "Physics engine failed to load: " + (err?.message || err);
    return;
  }
  ...
  installPfpExternalMatch(game, {
    ...
    onGameOver: ({ context, startedAt, endedAt, players, winner, reason }) => {
      pfpRuntime.client.gameOver(
        buildStickSmashResult({
          context,
          startedAt,
          endedAt,
          players,
