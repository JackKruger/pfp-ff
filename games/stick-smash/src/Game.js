...
export class Game {
  constructor(options = {}) {
    ...
    this.pfpRuntime = options.pfpRuntime ?? null;
    this._pfp = null;
    this._pfpGameOverReported = false;
    this.onPfpGameOver = null;
    this.input = new InputManager();
    if (this.pfpRuntime?.controls) {
      this.input.setPfpControls(new PfpControls(this.pfpRuntime.controls));
    }
  ...
  _startMatch({ character, name, bots, levelId, isOnline, asClient, localPlayerId, localMP = false, extras = null, pfpPlayers = null }) {
    this._cleanup();
    ...
    if (pfpPlayers) {
      const shellPlayers = [...pfpPlayers].sort((a, b) => a.slot - b.slot);
      const used = new Set();
      for (let i = 0; i < shellPlayers.length; i++) {
        const player = shellPlayers[i];
        let pick = ROSTER[player.slot % ROSTER.length] || ROSTER[i % ROSTER.length];
        if (used.has(pick.id)) pick = ROSTER.find((candidate) => !used.has(candidate.id)) || pick;
        used.add(pick.id);
        ...
        sm.pfpSlot = player.slot;
        sm.pfpProfileId = player.profileId;
  ...
  _checkGameOver() {
    ...
    if (this.localPlayers.length === 1) {
      ...
      if (local && local.lives <= 0 && local.state === STATE.DEAD) {
        this.running = false;
        if (this._finishPfpGameOver({ winner: null, reason: 'ko' })) return;
        audio.death();
    ...
    if (stillIn.length === 0) {
      this.running = false;
      if (this._finishPfpGameOver({ winner: null, reason: 'draw' })) return;
      audio.death();
    ...
    if (stillIn.length === 1) {
      ...
      this.running = false;
      if (this._finishPfpGameOver({ winner, reason: 'victory' })) return;
  ...
  _finishPfpGameOver({ winner, reason }) {
    if (!this._pfp || this._pfpGameOverReported) return false;
    this._pfpGameOverReported = true;
    this.onPfpGameOver?.({
      winner,
