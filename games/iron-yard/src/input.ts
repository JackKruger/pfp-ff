...
const KEYBOARD_PRESETS: KeyboardBinding[] = [
  { up: "w", down: "s", left: "a", right: "d", aimUp: "ArrowUp", aimDown: "ArrowDown", aimLeft: "ArrowLeft", aimRight: "ArrowRight", block: "f", sprint: "ShiftLeft", jump: " ", attack: "KeyE" },
  { up: "i", down: "k", left: "j", right: "l", aimUp: "KeyU", aimDown: "KeyJ", aimLeft: "KeyH", aimRight: "KeyK", block: "u", sprint: "o", jump: "p", attack: "KeyY" },
];
...
export class InputManager {
  ...
  getInput(playerIndex: number, playerYaw: number): PlayerInput {
    ...
    let mv = { x: 0, y: 0 };
    let aimDX = 0, aimDY = 0;
    let block = false, sprint = false, jump = false;
    ...
    if (aimMag > 1) { aimDX /= aimMag; aimDY /= aimMag; }

    // Detect attack type from aim direction + trigger
    let attackType: string | null = null;
    if (attackTriggered) {
      const adx = Math.abs(aimDX), ady = Math.abs(aimDY);
