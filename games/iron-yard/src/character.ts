...
export function buildCharacter(
  ...
  const UPPER_ARM_LEN = height * 0.2;
  ...
  const pelvis = new THREE.Mesh(
    new THREE.CylinderGeometry(radius * 0.78, radius * 0.62, height * 0.13, 14),
    plateMat,
  );
  ...
  belt.position.y = Y_PELVIS_TOP - height * 0.005;
  root.add(belt);
  ...
  function buildLeg(side: string) {
    ...
    thigh.position.set(sx * radius * 0.3, Y_HIP - height * 0.02, 0);
    const thighM = new THREE.Mesh(
      new THREE.CylinderGeometry(radius * 0.28, radius * 0.24, THIGH_LEN, 12),
      mailMat,
    );
    thighM.position.y = -THIGH_LEN / 2;
  ...
  const abdomen = new THREE.Mesh(
    new THREE.CylinderGeometry(radius * 0.72, radius * 0.78, height * 0.12, 14),
    mailMat,
  );
  abdomen.position.y = Y_ABDOMEN;
  ...
  const gorget = new THREE.Mesh(
    new THREE.CylinderGeometry(radius * 0.42, radius * 0.55, height * 0.06, 14),
    plateMat,
  );
  ...
  const neck = new THREE.Mesh(
    new THREE.CylinderGeometry(radius * 0.24, radius * 0.27, height * 0.07, 10),
    skinMat,
  );
  neck.position.y = Y_NECK - height * 0.025;
  root.add(neck);
  const head = new THREE.Mesh(new THREE.SphereGeometry(radius * 0.5, 16, 14), skinMat);
  ...
  slit.position.set(0, radius * 0.1, -radius * 0.48);
  helm.add(slit);
  ...
  const upperArmL = new THREE.Mesh(
    new THREE.CylinderGeometry(radius * 0.22, radius * 0.2, UPPER_ARM_LEN, 10),
    mailMat,
  );
  upperArmL.position.y = -UPPER_ARM_LEN / 2;
  ...
  const forearmL = new THREE.Mesh(
    new THREE.CylinderGeometry(radius * 0.2, radius * 0.18, FOREARM_LEN, 10),
    plateMat,
  );
  forearmL.position.y = -FOREARM_LEN / 2;
  forearmL.castShadow = true;
  elbowL.add(forearmL);
  const handL = new THREE.Mesh(
    new THREE.BoxGeometry(radius * 0.3, radius * 0.22, radius * 0.32),
    plateMat,
  );
  handL.position.y = -FOREARM_LEN - radius * 0.05;
  elbowL.add(handL);
  ...
  upperArmR.position.y = -UPPER_ARM_LEN / 2;
  upperArmR.castShadow = true;
  weaponRig.add(upperArmR);
  ...
  forearmR.position.y = -FOREARM_LEN / 2;
  forearmR.castShadow = true;
  elbowR.add(forearmR);
  const handR = new THREE.Mesh(
    new THREE.BoxGeometry(radius * 0.32, radius * 0.24, radius * 0.34),
    plateMat,
  );
  handR.position.y = -FOREARM_LEN - radius * 0.06;
  elbowR.add(handR);
  ...
  const trailMat = new THREE.LineBasicMaterial({
    color: TRAIL_COLOR[weaponKey] ?? 0xfff0c0,
    transparent: true,
    opacity: 0,
  });
  ...
  // Sword and shield meshes (mutable, for swapWeapon)
  let _currentSword: THREE.Group = sword;
  ...
  const rig: CharacterRig = {
    ...
    parts: {
      ...
      footLMesh: footLMesh!,
      footRMesh: footRMesh!,
      upperArmLMesh: upperArmL,
      upperArmRMesh: upperArmR,
      forearmLMesh: forearmL,
      forearmRMesh: forearmR,
      handLMesh: handL,
      handRMesh: handR,
      helmDome,
      visor,
    ...
    animate(dt, opts) {
      ...
      const lift = Math.max(-0.05, Math.min(0.2, verAim * 0.18));
      ...
      if (alive) {
        const breath = (1 - Math.min(1, mvSpeed)) * 0.03 * Math.sin(anim.swayPhase * 1.6);
        ...
        if (grip === "shield") {
          ...
          armL.position.set(-radius * 0.8, Y_SHOULDER - height * 0.02, -radius * 0.25);
          armL.rotation.x = -0.6;
          ...
          if (blocking) {
            ...
            _shieldMesh.position.set(0, -(UPPER_ARM_LEN + FOREARM_LEN) - 0.1, 0.15);
          } else {
            _shieldMesh.position.set(0, -(UPPER_ARM_LEN + FOREARM_LEN) - 0.15, 0);
          }
          ...
          if (d > 0.001)
            armL.quaternion.setFromUnitVectors(new THREE.Vector3(0, -1, 0), targetVec.normalize());
          elbowL.rotation.x =
            0.1 + Math.acos(Math.min(1, d / (UPPER_ARM_LEN + FOREARM_LEN))) * 0.55;
        } else {
          // One-hand: swing arm naturally
          armL.rotation.x = -Math.sin(phase) * 0.5 * stride;
          elbowL.rotation.x = 0.3 + Math.max(0, -Math.sin(phase) * 0.3 * stride);
        }
        ...
        if (stunned) {
          ...
          torso.rotation.x += Math.sin(anim.swayPhase * 5.0) * 0.1;
        ...
        anim.leanZ +=
          (Math.max(-0.55, Math.min(0.55, swingLat * 0.1)) - anim.leanZ) * Math.min(1, dt * 14);
        anim.leanX +=
          (Math.max(-0.45, Math.min(0.45, swingFwd * 0.08)) - anim.leanX) * Math.min(1, dt * 14);
        torso.rotation.z = idleSway + anim.leanZ;
        torso.rotation.x = anim.leanX;

        if (torsoRot) {
          const cy = Math.cos(-playerYaw),
            sy = Math.sin(-playerYaw);
          torso.rotation.x += Math.max(
            -0.18,
            Math.min(0.18, (cy * torsoRot.x + sy * torsoRot.z) * 0.7),
          );
          torso.rotation.z += Math.max(
            -0.18,
            Math.min(0.18, (-sy * torsoRot.x + cy * torsoRot.z) * 0.7),
          );
        }
        if (headRot) {
          const cy = Math.cos(-playerYaw),
            sy = Math.sin(-playerYaw);
          head.rotation.x = Math.max(-0.2, Math.min(0.2, (cy * headRot.x + sy * headRot.z) * 0.6));
          head.rotation.z = Math.max(-0.2, Math.min(0.2, (-sy * headRot.x + cy * headRot.z) * 0.6));
          helm.rotation.x = head.rotation.x;
          helm.rotation.z = head.rotation.z;
        }

        pelvis.rotation.y = -anim.leanZ * 1.6;
        pelvis.rotation.x = -anim.leanX * 0.6;
        const stagger = Math.max(-0.3, Math.min(0.3, anim.leanZ * 1.2));
        legL.thigh.position.z = stagger * 0.3;
        legR.thigh.position.z = -stagger * 0.3;
        ...
        if (attackT >= 0 && attackT <= 1) {
          ...
          if (attackT < 0.3) {
            const w = attackT / 0.3;
            crouch = -0.1 * Math.sin(w * Math.PI * 0.5);
            spineBend = -0.18 * w;
          } else if (attackT < 0.6) {
            const w = (attackT - 0.3) / 0.3;
            const arc = Math.sin(w * Math.PI);
            crouch = 0.05 * arc;
            surge = 0.2 * arc;
            spineBend = 0.32 * arc;
          } else {
            const w = (attackT - 0.6) / 0.4;
            spineBend = 0.1 * (1 - w);
          }
          ...
          if (
            ...
            const twist =
              attackT < 0.3
                ? -dir * 0.35 * (attackT / 0.3)
                : attackT < 0.6
                  ? -dir * 0.35 + dir * 0.85 * ((attackT - 0.3) / 0.3)
                  : dir * 0.5 * (1 - (attackT - 0.6) / 0.4);
            torso.rotation.y = twist;
            pelvis.rotation.y -= twist * 0.4;
          }
        }
      } else {
        // Death pose
    ...
    swapWeapon(newKey) {
      // Remove old sword
      if (_currentSword) weaponRig.remove(_currentSword);
